...
sys.path.append('/DATACENTER5/jicheng.liu/SWJTU_OOPAO/SWJTU_OOPAO/tutorials/scao_system/myAoSystem/RL4AO/agent/offline/AWAC/awac-adaptive-optics')
from agent.awac_v1 import AWAC
from utils.helpers import save_model
from envs.AdaptiveOpticsEnvR2DISC_v1 import AOEnv
...
def main(args):
    ...
    # --- 1. 初始化环境和智能体 ---
    save_dir = f"/DATACENTER5/jicheng.liu/SWJTU_OOPAO/SWJTU_OOPAO/tutorials/scao_system/myAoSystem/RL4AO/agent/offline/AWAC/awac_results/{datetime.now().strftime('%Y%m%d_%H%M%S')}"
    ...
    for npz_file in npz_files:
        try:
            ...
            # 提取数据流
            images_hwt = data['image']          # 形状: (N, H, W, T) -> (201, 64, 64, 3)
            ...
            # --- 核心修改：数据加载逻辑以适应新的预堆叠数据格式 ---
            # 循环到 len(...) - 1 以确保 next_state (i+1) 存在
            for i in range(len(images_hwt) - 1):
                # --- 构建当前状态 s_i ---
                # 数据已经是堆叠好的，我们只需要 reshape 和 transpose
                state_dict = {
                    # 1. 图像: 从 (H, W, T) 转换为 (T, H, W)
                    "far_img": np.transpose(images_hwt[i], (2, 0, 1)),
                    
                    # 2. WFS 信号: 从 (T*Dim) 重塑为 (T, Dim)
                    "slope_vec": wfs_signal_flat[i].reshape(slope_vec_shape),

                    # 3. DM 系数: 从 (T*Dim) 重塑为 (T, Dim)
                ...
                # 动作和奖励的索引保持不变 (s_i, a_i, r_{i+1}, s_{i+1})
...
if __name__ == '__main__': 
    ...
    # 训练流程参数
    parser.add_argument('--offline_dataset_dir', type=str, default='SWJTU_OOPAO/tutorials/scao_system/myAoSystem/RL4AO/agent/dreamerv3/logdir/AO4_R2DISC_magnitude0_reward_test_mlp4_lossweight_explutil0/train_eps', help='离线数据集目录路径')
    parser.add_argument('--offline_steps', type=int, default=10000, help='离线预训练的总步数')
    parser.add_argument('--online_steps', type=int, default=10000, help='在线微调的总步数')
    ...
    parser.add_argument('--lr', type=float, default=1e-5, help='优化器的学习率')
    parser.add_argument('--awac_lambda', type=float, default=5.0, help='AWAC优势加权的温度系数')
