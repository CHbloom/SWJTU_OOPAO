...
class OneHotMultiDiscreteAction(gym.Wrapper):
    def __init__(self, env):
        assert isinstance(env.action_space, gym.spaces.MultiDiscrete)
        super().__init__(env)
        self._random = np.random.RandomState()
        self.nvec = env.action_space.nvec
        self.total_dim = sum(self.nvec)
        self.action_space = gym.spaces.Box(low=0, high=1, shape=(self.total_dim,), dtype=np.float32)
        self.action_space.discrete = True

    def step(self, action):
        indices = self._decode_onehot(action)
        return self.env.step(indices)

    def reset(self):
        return self.env.reset()

    # def _decode_onehot(self, onehot):
    ...
    def _sample_action(self):
        ...
        segments = []
        for dim in self.nvec:
            index = self._random.randint(0, dim)
            onehot = np.zeros(dim, dtype=np.float32)
            onehot[index] = 1.0
            segments.append(onehot)
