...
class Dreamer(nn.Module):
    def __init__(self, obs_space, act_space, config, logger, dataset):
        ...
        print(torch.cuda.current_device(), config.device)  
        self._config = config
        self._logger = logger
...
def test_after_train(env,agent,config):
    ...
    for ep in trange(episodes, desc="Evaluating"):
        ...
        # np.save(f"{config.logdir}/test_after_train/SR_episode_{ep + 1}.npy", np.array(env.actual_SR)) 
        plt.figure()
        plt.plot(np.arange(env.max_step), env.actual_SR, label='Strehl Ratio')
        plt.xlabel('Frame')
        plt.title(f'Strehl Ratio(return={total_reward:.2f},avg_SR={sum(env.actual_SR)/len(env.actual_SR):.2f})')
        plt.legend()
        plt.grid()
...
if __name__ == "__main__":
    ...
    parser.add_argument("--paramFile", type=str, default="paramFile4", help="paramFile")
