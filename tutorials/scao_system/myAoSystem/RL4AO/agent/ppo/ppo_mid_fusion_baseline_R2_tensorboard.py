...
def parse_args():
    ...
    parser.add_argument("--max_step", type=int, default=500, help="AOEnv max_step")
    ...
    parser.add_argument("--paramFile", type=str, default="1-1", help="paramFile")
    ...
    def __init__(self, obs_space):
        ...
        for key, space in obs_space.spaces.items():
            if "img" in key:
                ...
                # 使用 LazyLinear 延迟初始化全连接层，避免占位观察空间与实际帧尺寸不一致导致的维度错配
                cnn_feature_dim = 128
                self.cnn_fcs[key] = nn.LazyLinear(cnn_feature_dim)
...
if __name__=="__main__":
    ...
    tb_logdir = os.path.join("runs/2", save_dir, timestamp)
    ...
    for i in range(10):
        with tqdm(total=int(episodes/10), desc="Iteration %d" % i) as pbar:
            for episode in range(int(episodes/10)):
                ...
                # log to TensorBoard
                writer.add_scalar("train/episode_return", episode_return, total_episode_counter)
                writer.add_scalar("train/avg_SR", np.mean(episode_sr), total_episode_counter)
                writer.add_scalar("train/avg_SR_old", np.mean(episode_sr_old), total_episode_counter)
                writer.add_scalar("train/avg_WFE", np.mean(episode_wfe), total_episode_counter)
                writer.add_scalar("train/episode_length", len(transition_dict["rewards"]), total_episode_counter)
