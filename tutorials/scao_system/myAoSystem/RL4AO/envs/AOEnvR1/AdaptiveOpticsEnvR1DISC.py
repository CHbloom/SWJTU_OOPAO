...
class AOEnv(gym.Env):
    def __init__(self, args):
        ...
        self.observation_space = spaces.Box(
            ...
            shape=(120*120 + 664 + 357 + 4 + 3,), dtype=np.float32
        )
    ...
    def reset(self, seed: Optional[int] = None, options: Optional[dict] = None):
        ...
        self.observation_space = spaces.Box(
            low=-np.inf, high=np.inf, 
            shape=(self.camH.frame.shape[0]*self.camH.frame.shape[1] + self.wfs.nSignal + self.dm.nValidAct + 4 + 3,), dtype=np.float32
        )
    ...
    def _calculate_reward(self, action, current_wfe):
        w = {
            "base_reward_scale": 2.0,       # 基础奖励放大倍数
        ...
        # 新增湍流波动惩罚
        print("wfe_history_size:", len(self.wfe_history))
        window_size = 5
