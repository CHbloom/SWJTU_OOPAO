...
class AOEnv(gym.Env):
    def __init__(self, args):
        ...
        # --- 核心修改 1: 将 observation_space 定义为 Dict ---
        # 注意：这里的维度是示例，实际维度会在第一次reset后动态获取并填充
        # PPO算法会在第一次reset后获取正确的space，所以这里的初始值仅为占位
        self.observation_space = spaces.Dict({
            "wfs_img": spaces.Box(low=-np.inf, high=np.inf, shape=(self.time_window, self.tel.resolution, self.tel.resolution), dtype=np.float32),
            "far_img": spaces.Box(low=-np.inf, high=np.inf, shape=(self.time_window, self.tel.resolution, self.tel.resolution), dtype=np.float32),
            "slope_vec": spaces.Box(low=-np.inf, high=np.inf, shape=(self.time_window, self.wfs.nSignal), dtype=np.float32),
            "dm_vec": spaces.Box(low=-np.inf, high=np.inf, shape=(self.time_window, self.dm.nValidAct), dtype=np.float32)
        })
    ...
    def _get_state(self):
        """ --- 核心修改 2: 构建并返回一个状态字典 --- """
        # 动态归一化
        wfs_frames = np.array([self._normalize_wfs(f) for f in self.wfs_buffer])
        far_frames = np.array([self._normalize_far(f) for f in self.far_buffer])

        # 返回一个字典，而不是拼接后的向量
        state_dict = {
            "wfs_img": wfs_frames.astype(np.float32),      # Shape: (time_window, H, W)
            "far_img": far_frames.astype(np.float32),      # Shape: (time_window, H, W)
            "slope_vec": np.array(self.slope_buffer, dtype=np.float32), # Shape: (time_window, nSignal)
            "dm_vec": np.array(self.dm_buffer, dtype=np.float32)        # Shape: (time_window, nValidAct)
        }
        # 直接返回字典，移除错误的 .astype() 调用
        return state_dict
    ...
    def step(self, action):
        ...
        self.dm.coefs -= self.gainCL * self.M2C_CL @ self.calib_CL.M @ self.wfsSignal
    ...
    def _calculate_reward(self, action, current_wfe):
        w = {
            ...
            #降低惩罚权重
            "gain_penalty_scale": 0.01,
            "freq_gain_penalty_scale": 0.01,
            "freq_expo_penalty_scale": 0.02,
            "clock_penalty_scale": 0.01,
        }
        ...
        # --- 保留弱化的惩罚项 ---
        # 约束1: 曝光时间↑ → 增益↓
        gain_penalty = np.abs(gain - (1.0 - 0.9 * (exposure - 0.1) / 0.9))
    ...
    def reset(self, seed: Optional[int] = None, options: Optional[dict] = None):
        ...
        # --- 核心修改 3: 移除此处的 observation_space 定义 ---
        # self.observation_space = spaces.Box(...) # <--- 删除这一整块

        # 第一次reset时，根据真实维度更新Dict Space
        if not isinstance(self.observation_space, spaces.Dict):
             self.observation_space = spaces.Dict({
                "wfs_img": spaces.Box(low=-np.inf, high=np.inf, shape=(self.time_window, self.wfs.cam.frame.shape[0], self.wfs.cam.frame.shape[1]), dtype=np.float32),
                "far_img": spaces.Box(low=-np.inf, high=np.inf, shape=(self.time_window, self.camH.frame.shape[0], self.camH.frame.shape[1]), dtype=np.float32),
                "slope_vec": spaces.Box(low=-np.inf, high=np.inf, shape=(self.time_window, self.wfs.nSignal), dtype=np.float32),
                "dm_vec": spaces.Box(low=-np.inf, high=np.inf, shape=(self.time_window, self.dm.nValidAct), dtype=np.float32)
            })
