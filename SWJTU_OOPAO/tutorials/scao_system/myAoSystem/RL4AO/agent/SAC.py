...
class PolicyNetContinuous(torch.nn.Module):
    def __init__(self, state_dim, hidden_dim, action_dim):
        ...
        action_bounds = [
         ...
         # 动作边界 [low, high] (shape: [action_dim, 2])
    ...
    def forward(self, x):
        ...
        std = F.softplus(self.fc_std(x)) + 1e-4  # 避免过小的std
        dist = Normal(mu, std)
        ...
        # 计算对数概率（修正tanh变换的影响）
        log_prob = dist.log_prob(normal_sample) - torch.log(1 - action_tanh.pow(2) + 1e-6)
        
        # 将动作映射到实际范围 [low, high]
        action = self.action_bounds[:, 0] + (action_tanh + 1) * 0.5 * (self.action_bounds[:, 1] - self.action_bounds[:, 0])
