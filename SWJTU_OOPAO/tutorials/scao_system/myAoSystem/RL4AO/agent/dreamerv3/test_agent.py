...
def main(config, args):
    ...
    for ep in trange(episodes, desc="Evaluating"):
        ...
        np.save(f"{config.logdir}/test/SR_episode_{ep + 1}.npy", np.array(env.SR)) 
        plt.figure()
        ...
        plt.legend()
        plt.grid()
        plt.savefig(f"{config.logdir}/test/SR_episode_{ep + 1}.png")
        plt.close()
...
if __name__ == "__main__":
    ...
    parser.add_argument("--lightRatio", type=float, default=0.7, help="wfs lightRatio")
