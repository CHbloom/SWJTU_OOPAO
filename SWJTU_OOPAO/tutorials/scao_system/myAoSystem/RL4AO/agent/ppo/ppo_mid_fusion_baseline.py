...
def parse_args():
    ...
    parser.add_argument("--agent", type=str, default="ppo_mid_fusion_R1", choices=["PPO", "SAC", "Dreamer-v3"], help="Reinforcement learning algorithm")
    ...
    def __init__(self, obs_space):
        ...
        for key, space in obs_space.spaces.items():
            if "img" in key:
                ...
                # 动态计算CNN输出维度并创建FC层
                with torch.no_grad():
                    dummy_input = torch.zeros(1, 1, *img_shape)
                    cnn_output_dim = cnn(dummy_input).shape[1]
                
                cnn_feature_dim = 128  # 可以为每个图像输出设定不同的维度
                self.cnn_fcs[key] = nn.Linear(cnn_output_dim, cnn_feature_dim)
                total_features_dim += cnn_feature_dim

            elif "vec" in key:
                # 为每个向量输入创建一个MLP
                vec_dim = space.shape[0]
                mlp_feature_dim = 64 # 可以为每个向量输出设定不同的维度
                mlp = nn.Sequential(
