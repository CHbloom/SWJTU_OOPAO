...
class AOEnv(gym.Env):
    def __init__(self, args):
        ...
        self.observation_space = spaces.Dict({
            "img": spaces.Box(low=0, high=1, shape=(self.tel.resolution, self.tel.resolution), dtype=np.float32),
            "vec": spaces.Box(low=-1, high=1, shape=(self.wfs.nSignal + 3,), dtype=np.float32)
        })
    ...
    def _get_state(self):
        ...
        # 使用最大绝对值进行归一化，更稳定
        wfs_norm = self.wfsSignal / (np.max(np.abs(self.wfsSignal)) + 1e-8)
        frame_norm = self.camH.frame / (self.camH.frame.max() + 1e-8)

        # 裁剪以确保值在定义域内
        wfs_norm = np.clip(wfs_norm, -1.0, 1.0)
        frame_norm = np.clip(frame_norm, 0.0, 1.0)

        # 拼接向量部分
        vec_state = np.concatenate([
            wfs_norm.flatten(),
            np.array([self.gainCL, samplingTime_norm, clock_rate_norm])
        ]).astype(np.float32)

        # 返回字典形式的状态
        return {
            "img": frame_norm.astype(np.float32),
            "vec": vec_state
        }
