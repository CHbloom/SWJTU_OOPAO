...
class Atmosphere:
    def __init__(self,telescope,r0:float,L0:float,windSpeed:list,fractionalR0:list,windDirection:list,altitude:list,mode:float=2, param = None, asterism = None):
        ...
        ************************** FUNCTIONS **************************
        ...
        self.r0_def                 = 0.15      # 默认的 Fried 参数（r0），表示大气相干长度，单位为米（m）
        self.r0                     = r0                # Fried 参数（r0），表示大气相干长度，单位为米
        self.fractionalR0           = fractionalR0      # 每个湍流层的 Cn² 剖面，表示每个层对总湍流的贡献比例
        self.L0                     = L0                # 外尺度是描述大气湍涡的最大尺度。通常在几十米到几百米之间
        self.altitude               = altitude          # 每层大气的高度，单位为米（m）
        self.nLayer                 = len(fractionalR0)     # 大气层数
        self.windSpeed              = windSpeed         # 每层大气的风速 m/s
        self.windDirection          = windDirection     # 每层大气的风向，单位为度
        self.tag                    = 'atmosphere'
        self.nExtra                 = 2                 # 生成相位屏时，额外添加的像素数
        self.wavelength             = 500*1e-9          # 大气属性的波长，单位为米
