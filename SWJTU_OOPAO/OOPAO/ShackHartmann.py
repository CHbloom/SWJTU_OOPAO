...
class ShackHartmann:
    def __init__(self, nSubap: float, telescope, lightRatio: float, threshold_cog: float = 0.01, \
        ...
        ************************** PROPERTIES **************************
        ...
        self.telescope = telescope  # 关联的望远镜对象
        self.is_geometric = is_geometric  # 是否启用几何模式（直接测量梯度）
        self.nSubap = nSubap  # 沿望远镜入瞳直径的子孔径数量
        self.lightRatio = lightRatio  # 用于筛选有效子孔径
        self.binning_factor = binning_factor  # 像素合并因子
        self.padding_extension_factor = padding_extension_factor # 光斑图像的零填充因子，用于防止卷积时的混叠效应
        self.threshold_convolution = threshold_convolution # 强制高斯光斑在边缘处归零的阈值
        self.threshold_cog = threshold_cog  # 计算光斑质心时的阈值
        self.shannon_sampling = shannon_sampling  # 是否以 Shannon 采样率（2 像素/FWHM）采样光斑
        # self.nValidSubaperture # 有效子孔径的数量
        # self.signal # 测量的波前信号（1D 数组）(2*nValidSubaperture,)
        # self.signal_2D # 测量的波前信号（2D 数组）(2 * nSubap, nSubap)
        self.zero_padding = 2
        ...
        self.lenslet_frame = np.zeros([self.n_pix_subap * self.zero_padding, self.n_pix_subap * self.zero_padding],
                                      dtype=complex)
        ...
        # field of views
        self.fov_lenslet_arcsec = (
                                              self.n_pix_subap * 206265 * self.binning_factor / self.padding_extension_factor * self.telescope.src.wavelength / (
                                                  self.telescope.D / self.nSubap)) / (1 + self.shannon_sampling)
        ...
        X_map, Y_map = np.meshgrid(np.arange(self.n_pix_subap // self.binning_factor),
                                   np.arange(self.n_pix_subap // self.binning_factor))
        ...
        # camera frame 
        self.camera_frame = np.zeros([self.n_pix_subap * (self.nSubap) // self.binning_factor,
        ...
        self.valid_subapertures = np.reshape(
            self.photon_per_subaperture >= self.lightRatio * np.max(self.photon_per_subaperture),
            [self.nSubap, self.nSubap])
    ...
    def initialize_flux(self, input_flux_map=None):
        if self.telescope.tag != 'asterism':
            ...
            for i in range(self.nSubap):
                ...
                self.center_init - self.n_pix_subap_init // 2:self.center_init + self.n_pix_subap_init // 2] = np.asarray(
                    tmp_flux_v_split)
    ...
    def get_lenslet_em_field(self, phase):
        ...
        for i in range(self.nSubap):
            ...
            self.center_init - self.n_pix_subap_init // 2:self.center_init + self.n_pix_subap_init // 2,
            self.center_init - self.n_pix_subap_init // 2:self.center_init + self.n_pix_subap_init // 2] = np.exp(
                1j * np.asarray(tmp_phase_v_split))
    ...
    def compute_camera_frame_multi(self, maps_intensity):
        ...
        def joblib_fill_camera_frame():
            Q = Parallel(n_jobs=1, prefer='processes')(delayed(self.fill_camera_frame)(i, j, k, l) for i, j, k, l in
                                                       zip(index_x, index_y, self.maps_intensity, self.ind_frame))
            return Q
    ...
    def get_convolution_spot(self):
        ...
        [X0, Y0] = [self.telescope.src.laser_coordinates[1], -self.telescope.src.laser_coordinates[
            0]]  # coordinates of the LLT in [m] from the center (sign convention adjusted to match display position on camera)
        ...
        for i in range(len(self.telescope.src.Na_profile[0, :])):
            ...
            coordinates_3D[2, i] = self.telescope.D ** 2. / (8. * self.telescope.src.Na_profile[0, i]) / (
                        2. * np.sqrt(3.))
            coordinates_3D_ref[:, i] = coordinates_3D[:, i] - coordinates_3D[:,
                                                              len(self.telescope.src.Na_profile[0, :]) // 2]
        C_elung = []
        C_gauss = []
        shift_x_buffer = []
        shift_y_buffer = []
        ...
        I_gauss = (self.telescope.src.Na_profile[1, :][0] / (self.telescope.src.Na_profile[0, :][0] ** 2)) * np.exp(
            - ((alpha_x) ** 2 + (alpha_y) ** 2) / (2 * sigma_spot ** 2))
        I_gauss /= I_gauss.sum()
        ...
        for i_subap in range(len(x_subap)):
            for j_subap in range(len(y_subap)):
                ...
                if valid_subap_1D[count]:
                    ...
                    for i in range(len(self.telescope.src.Na_profile[0, :])):
                        coordinates_3D[:2, i] = (self.telescope.D / 4) * (
                                    [X0, Y0] / self.telescope.src.Na_profile[0, i])
                        coordinates_3D[2, i] = self.telescope.D ** 2. / (8. * self.telescope.src.Na_profile[0, i]) / (
                                    2. * np.sqrt(3.))
                        ...
                        delta_dx[1, i] = coordinates_3D_ref[2, i] * (np.sqrt(3) * (4 / self.telescope.D) ** 2) * \
                                         x_subap[i_subap]
                        delta_dy[1, i] = coordinates_3D_ref[2, i] * (np.sqrt(3) * (4 / self.telescope.D) ** 2) * \
                                         y_subap[j_subap]
                        ...
                        I_tmp = (self.telescope.src.Na_profile[1, :][i] / (
                                    self.telescope.src.Na_profile[0, :][i] ** 2)) * np.exp(
        ...
        if self.max_elung_x > criterion_elungation or self.max_elung_y > criterion_elungation:
            print(
                '!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!  WARNING  !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!')
            print('Warning: The largest spot elongation is ' + str(np.round(self.elungation_factor,
                                                                            3)) + ' times larger than a subaperture! Consider using a higher resolution or increasing the padding_extension_factor parameter')
            print(
                '!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!')
        ...
        if self.is_geometric is False:
            ...
            if np.ndim(self.telescope.OPD) == 2:
                ...
                if self.edge_subaperture_criterion > 0.05:
                    print(
                        '%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%')
                    print('WARNING !!!! THE LIGHT IN THE SUBAPERTURE IS MAYBE WRAPPING !!!' + str(
                        np.round(100 * self.edge_subaperture_criterion,
                                 1)) + ' % of the total flux detected on the edges of the subapertures. You may want to increase the seeing value or the resolution')
                    print(
                        '%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%')
                ...
                if self.shannon_sampling:
                    self.maps_intensity = I[:, self.n_pix_subap // 2:-self.n_pix_subap // 2,
                                          self.n_pix_subap // 2:-self.n_pix_subap // 2]
                    if self.binning_factor > 1:
                        self.maps_intensity = bin_ndarray(self.maps_intensity, [self.maps_intensity.shape[0],
                                                                                self.n_pix_subap // self.binning_factor,
                                                                                self.n_pix_subap // self.binning_factor],
                                                          operation='sum')
                else:
                    self.maps_intensity = bin_ndarray(I, [I.shape[0], self.n_pix_subap // self.binning_factor,
                ...
                if self.cam.readoutNoise != 0:
                    self.maps_intensity += np.int64(np.round(
                        self.random_state_readout_noise.randn(self.maps_intensity.shape[0],
                                                              self.maps_intensity.shape[1],
                                                              self.maps_intensity.shape[2]) * self.cam.readoutNoise))

                # fill camera frame with computed intensity (only valid subapertures)
                def joblib_fill_camera_frame():
                    Q = Parallel(n_jobs=1, prefer='processes')(delayed(self.fill_camera_frame)(i, j, k) for i, j, k in
                                                               zip(self.index_x[self.valid_subapertures_1D],
                ...
                self.camera_frame = np.zeros(
                    [self.phase_buffer.shape[0], self.n_pix_subap * (self.nSubap) // self.binning_factor,
                     self.n_pix_subap * (self.nSubap) // self.binning_factor], dtype=float)
                ...
                emf_buffer = np.reshape(np.asarray(compute_diffractive_signals_multi()),
                                        [self.phase_buffer.shape[0] * self.nSubap ** 2, self.n_pix_lenslet_init,
                ...
                self.maps_intensity = (
                    bin_ndarray(I, [I.shape[0], self.n_pix_subap, self.n_pix_subap], operation='sum'))
                # add photon/readout noise to 2D spots
                if self.cam.photonNoise != 0:
                    self.maps_intensity = self.random_state_photon_noise.poisson(self.maps_intensity)
                ...
                for i in range(self.phase_buffer.shape[0]):
                    self.SX[self.validLenslets_x, self.validLenslets_y] = self.centroid_lenslets[
                                                                          i * self.nValidSubaperture:(
                                                                                                                 i + 1) * self.nValidSubaperture,
                                                                          0]
                    self.SY[self.validLenslets_x, self.validLenslets_y] = self.centroid_lenslets[
                                                                          i * self.nValidSubaperture:(
                                                                                                                 i + 1) * self.nValidSubaperture,
                                                                          1]
                    signal_2D = np.concatenate((self.SX, self.SY)) - self.reference_slopes_maps
    ...
    def print_properties(self):
        ...
        if self.is_LGS:
            print('{: ^20s}'.format('Spot Elungation') + '{: ^18s}'.format(
                str(100 * np.round(self.elungation_factor, 3))) + '{: ^18s}'.format('% of a subap'))
